...
3. follow the equation to compute the attended output of each head, applying a causal mask so each position can only attend to itself and previous positions
...
4. a "generate" function which uses temperature sampling with top-p (nucleus) sampling to generate long continuation strings.
...
* ```addition_data_generation.generate_addition_data```
* ```addition_run.train_one_epoch```
* ```addition_run.evaluate_loss```
...
## addition_data_generation.py (to be implemented)
This file contains the data generation function that you will use to create your addition training data. You need to implement the generate_addition_data() function in this file. 

## addition_lib.py
This contains datasets class to help train addition models. You won't need to modify this file in this assignment. 

## addition_run.py (to be implemented)
This file contains importnat functions needed for the second part of this assignment, where your Llama model learns to perform 4-digit addition. You will also use this file to run different
ablations and train your models. 
